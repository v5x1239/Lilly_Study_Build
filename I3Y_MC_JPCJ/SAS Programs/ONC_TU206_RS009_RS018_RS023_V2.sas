...
CODE NAME (required)              : ONC_TU206_RS009_RS018_RS023_V2.1.sas
PROJECT NAME (required)           : GLOBAL
...
DATA INPUT                        : ONC_TU206_RS009_RS018_RS023_V2.1.sql
OUTPUT                            : CSV
...
proc sql;
	create table rs as
	select g.subjid, rsspid,blockid, blockrpt,
	case 
	...
	end as TULNKID, OVRLRESP, TRGRESP,NTRGRESP,SYMPDET 
	from  clntrial.RS1001_V g 
...
proc sql;
	...
	select distinct a.*, rsspid,OVRLRESP,TRGRESP,NTRGRESP,SYMPDET
...
proc sql;
	...
	select distinct STUDY,SITEMNEMONIC, r.SUBJID,TRSPID,RSSPID,TULNKID,TRPERF,TRDAT,TULOC1 as TULOC,
	TULOCOTH,TRMETHOD,TRMETHODOTH,TUMSTATE,TRSTRESC,LDIAM,SAXIS,SUMDIAM,PCBSD,PCNSD,CRF_TRGRESP,CALC_TRGRESP,CRF_NTRGRESP,
	SYMPDET,OVRLRESP as CRF_OVRRESP,NEWTUMOR,
	catx ('',er1,er2,er3,er4,er5,er6,er7,er8,er9,er10,er11,er12,er13,er14,er15,er16) as Error_Message length=4000,
	...
	select c.*,case when LONG_DESC_TEXT is not null then LONG_DESC_TEXT else c.TULOC end as TULOC1,
		...
		CASE WHEN SYMPDET = "Y" AND OVRLRESP ne 'PD' THEN '|SYMPDET=Y, HOWEVER OVERALL RESPONSE IS NOT PD| '
		else ''  END AS er15,
			CASE
		...
		from (
/*Main Shell*/
select x.SUBJID, x.TRSPID,RSSPID,x.TULNKID, TRPERF,TRDAT,TULOC,TULOCOTH, TRMETHOD,TRMETHODOTH,TUMSTATE,TRSTRESC,LDIAM,SAXIS,
	g.SUMDIAM,
	 case when base_SUMDIAM ne 0 then round(((g.SUMDIAM-base_SUMDIAM)/base_SUMDIAM)*100) else . end as PCBSD ,
			case when small ne 0 then round (((g.SUMDIAM-small)/small)*100) else . end  as PCNSD , 
				...
				case when x.TRSPID ne 1 and i.subjid is not null and x.TULNKID='T01' then 'NE' 
					...
					else '' end as CALC_TRGRESP, NTRGRESP as CRF_NTRGRESP, SYMPDET,OVRLRESP, NEWTUMOR
...
/*	(select subjid, rsspid from rs) b on a.subjid=b.subjid and a.trspid=b.rsspid where min_trdat ne RSDAT) q on */
...
left join (select subjid, rsspid from (select s.subjid, RSSPID,OVRLRESP,TRGRESP,NTRGRESP, v.TULNKID,
	...
	v.TULNKID is null and SYMPDET="N" then 'x' 
	...
	v.TULNKID is null and SYMPDET="N" then 'x' else '' end as flag from rs s left join clntrial.TU1001_D v 
...
left join (select subjid, rsspid from 
	(select s.*, 
		case 
			...
			('NON CR OR NON PD', 'NOT ASSESSED', 'NON-CR OR NON-PD') and  v.TULNKID is null and SYMPDET="N" then 'x' 
			when w.subjid is not null and z.subjid is not null and OVRLRESP='PR' and TRGRESP='PR' and upcase (NTRGRESP) in 
			/**** Added 'CR' as per new update recived on 29AUG2017 from Robin***/
			('NON CR OR NON PD', 'NOT ALL EVALUATED', 'NON-CR OR NON-PD', 'CR') and  v.TULNKID is null and SYMPDET="N" then 'x' 
...
left join (select distinct subjid, rsspid from 
		...
		and v.TULNKID is null and SYMPDET="N" then 'x'
		...
		upcase (NTRGRESP) in ('NON-CR OR NON-PD','NON CR OR NON PD') and v.TULNKID is null and SYMPDET="N" then 'x'
...
left join (select distinct subjid, rsspid from 
	(select s.subjid, RSSPID,OVRLRESP,TRGRESP,NTRGRESP, v.TULNKID,
		  CASE WHEN OVRLRESP = 'PD' AND (TRGRESP = 'PD' OR NTRGRESP = 'PD' OR v.TULNKID IS NOT NULL OR (SYMPDET ='Y' AND (TRGRESP ne 'SD'
				OR NTRGRESP ne 'SD'))) THEN 'x' END AS flag
	from rs  s left join clntrial.TU1001_D v on s.SUBJID=v.subjid and s.RSSPID=v.TRSPID) 
...
left join (select distinct subjid, rsspid from 
	...
	case 
		...
		upper (NTRGRESP) in  ('NON CR OR NON PD', 'NOT ALL EVALUATED', 'NOT ASSESSED', 'NON-CR OR NON-PD', 'CR') or 
		v.TULNKID  is null) or  SYMPDET="N" or 
...
left join (select distinct a.subjid, a.rsspid from (select * from clntrial.RS1001_V where OVRLRESP='CR') a left join 
(select * from clntrial.RS1001_V where OVRLRESP in ('PR', 'SD')) b on a.subjid=b.subjid where a.rsspid < b.rsspid) 
...
left join (select distinct a.subjid,rs as RSSPID from (select subjid,OVRLRESP, count (*) as cn from clntrial.RS1001_V 
where OVRLRESP in ('CR', 'PR') group by subjid,OVRLRESP
having count (*) = 1) a left join (select  subjid, min(RSSPID) as rs from clntrial.RS1001_V where OVRLRESP in ('CR', 'PR') 
...
left join (select distinct a.subjid, rsspid from clntrial.RS1001_V a left join clntrial.TU3001_D b
...
left join (select distinct STUDY, SITE as SITEMNEMONIC,subjid from clntrial.EXPRHEAD) q on r.subjid=q.subjid 
...
proc sql;
	create table ONC_TU206_RS009_RS018_RS023_V2 as
	...
	SYMPDET,CRF_OVRRESP,NEWTUMOR,Error_Message from master3
...
proc print data=ONC_TU206_RS009_RS018_RS023_V2 noobs WIDTH=min; 
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set ONC_TU206_RS009_RS018_RS023_V2 nobs=NUM; 
if NUM=0 then do;

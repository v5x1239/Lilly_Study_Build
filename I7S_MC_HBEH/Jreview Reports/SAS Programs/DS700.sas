...
DESCRIPTION (required)            : Dates on rest of CRF must not be after the date of disposition.
...
DATA FINAL;
SET FIN;
IF DSSTDAT NE . AND DATE NE . AND DATE > DSSTDAT then fLAG = 'Date '||trim(DATE_VARIABLE)||' is after Discontinuation date';
RUN;
PROC SQL;
CREATE TABLE DS700 AS SELECT SITEMNEMONIC, SUBJID, DSSTDAT 'Discontinuation date', DATE, DATE_VARIABLE, PAGE, flag FROM final;
QUIT;
ods csv file=&irfilcsv trantab=ascii;
  title1 "DS";
	title2 "Dates on rest of CRF must not be after the date of disposition.";

proc print data=DS700 noobs label WIDTH=min;
    var _all_;
  run;

...
CODE NAME (required)              : eCOA725.sas
PROJECT NAME (required)           : I8B-MC-ITRM
DESCRIPTION (required)            : Check that Visit 18 MMTT lab collection date (Test Code I88) is the same as Visit 18 Meal Start Date
...
where VISITNUM = 18 and MLSTDAT ne '';
...
set cluwe.clrm_lab;
where TSTCD = 'I88' and VISITNUM = '18' and LBDTM ne .;
...
ods csv file=&irfilcsv trantab=ascii;
  title1 "eCOA725";
  title2 "Check that Visit 18 MMTT lab collection date (Test Code I88) is the same as Visit 18 Meal Start Date";

proc print data=eCOA725 noobs label WIDTH=min;
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set eCOA725 nobs=NUM;
if NUM=0 then do;

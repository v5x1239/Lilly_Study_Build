...
CODE NAME (required)              : eCOA014.sas
PROJECT NAME (required)           : I8B-MC-ITRM
DESCRIPTION (required)            : EQ5D should be present for visits 2, 8, and 18 where corresponding visits are continuing in Inform"
...
ods csv file=&irfilcsv trantab=ascii;
  title1 "eCOA014";
  title2 "EQ5D should be present for visits 2, 8, and 18 where corresponding visits are continuing in Inform";
proc print data=eCOA014 noobs label WIDTH=min;
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set eCOA014 nobs=NUM;
if NUM=0 then do;

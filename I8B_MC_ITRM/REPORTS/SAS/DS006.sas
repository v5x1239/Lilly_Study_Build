...
id = AEGRPID_RELREC;
if DSDECOD in ('ADVERSE EVENT');
keep SUBJID BLOCKID DSSTDAT1 DSDECOD DSCAT_1 DSSCAT AEGRPID_RELREC id;
run;
proc sort;
by subjid id;
run;
...
run;
data DS006;
retain SITEMNEMONIC SUBJID BLOCKID DSSTDAT DSDECOD DSCAT_1 DSSCAT AEGRPID_RELREC
...
if AESTDAT ne . and DSSTDAT ne . and AESTDAT > DSSTDAT then flag = 'latest AESTDAT > DSSTDAT';
keep SITEMNEMONIC SUBJID BLOCKID DSSTDAT DSDECOD DSCAT_1 DSSCAT AEGRPID_RELREC
...
ods csv file=&irfilcsv trantab=ascii;
  ...
  title2 "If  status is “Adverse event”, AE start date and/or Start Date of last AE status change (e.g.. Severity change)
  for the AE Group ID entered must be <= (Disposition Event) date";

proc print data=DS006 noobs label WIDTH=min;
    var _all_;
  run;

...
CODE NAME (required)              : eCOA728.sas
PROJECT NAME (required)           : I8B-MC-ITRM
DESCRIPTION (required)            : Check that Visit 8 MMTT Insulin Dose is the same as Visit 8 Meal Start Date
...
ods csv file=&irfilcsv trantab=ascii;
  title1 "eCOA728";
  title2 "Check that Visit 8 MMTT Insulin Dose is the same as Visit 8 Meal Start Date";

proc print data=eCOA728 noobs label WIDTH=min;
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set eCOA728 nobs=NUM;
if NUM=0 then do;

...
CODE NAME (required)              : eCOA726.sas
PROJECT NAME (required)           : I8B-MC-ITRM
DESCRIPTION (required)            : Check MMTT Insulin Dose date/time is between 5 minutes prior to  Meal Start Date/time to 30 minutes after Meal Start Date/time for same visit
...
by SUBJID VISITNUM ;
if a and b and MLSTDAT = EXSTDAT and (((EXSTTIM + 300) < MLSTTIM) or (EXSTTIM - 1800 >  MLSTTIM));
run;

ods csv file=&irfilcsv trantab=ascii;
  title1 "eCOA726";
  title2 "Check MMTT Insulin Dose date/time is between 5 minutes prior to  Meal Start Date/time to 30 minutes after Meal Start Date/time for same visit";

proc print data=eCOA726 noobs label WIDTH=min;
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set eCOA726 nobs=NUM;
if NUM=0 then do;

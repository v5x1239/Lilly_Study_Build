...
BGDAT label 'BG Monitoring Date: Before', BGDAT1 label 'BG Monitoring Date: 1hr After', BGDAT2 label 'BG Monitoring Date: 2hr After',
input('17JUL2017',date9.) as STUDY_START_DATE format date9. label 'Study Start Date' from
(select subjid, sitecode, ECOASTDT, input(catx("-",substr(bgdat,6,2),substr(bgdat,9,2),substr(bgdat,1,4)),mmddyy10.) as bgdat format date9.,
 input(catx("-",substr(bgdat1,6,2),substr(bgdat1,9,2),substr(bgdat1,1,4)),mmddyy10.) as bgdat1 format date9.,
input(catx("-",substr(bgdat2,6,2),substr(bgdat2,9,2),substr(bgdat2,1,4)),mmddyy10.) as bgdat2 format date9., 
bgconc, bgtim, lbtptref from cluwe.bg2001_ecoa)
where ((bgdat is not null and bgdat < input('17JUL2017',date9.)) OR (bgdat1 is not null and bgdat1 < input('17JUL2017',date9.))
OR (bgdat2 is not null and bgdat2 < input('17JUL2017',date9.))) order by subjid, bgdat, bgdat1, bgdat2; quit;

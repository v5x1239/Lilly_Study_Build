...
CODE NAME (required)              : eCOA711.sas
PROJECT NAME (required)           : I8B-MC-ITRN
DESCRIPTION (required)            : If V8 is present in InForm, a Visit 8 MMTT record should be present in eCOA.
...
where sv.subjid = stat.subjid and sv.blockid = stat.blockid and sv.blockid = '8' and stat.stat = '101'
...
ods csv file=&irfilcsv trantab=ascii;
  title1 "eCOA711";
  title2 "If V8 is present in InForm, a Visit 8 MMTT record should be present in eCOA.";

proc print data=eCOA711 noobs label WIDTH=min;
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set eCOA711 nobs=NUM;
if NUM=0 then do;

...
DESCRIPTION (required)            : Ensure there are not responses without Tumor data to support them. Except for Non-Radiological Progression..
...
/*	strip(put(datepart(MERGE_DATETIME),date9.)) as MERGE_DATETIME,*/
	...
	where TRSPID NE RSSPID /*and datepart(MERGE_DATETIME) > input("&date",Date9.))*/
...
ods csv file=&irfilcsv trantab=ascii;
  title1 "Ensure there are not responses without Tumor data to support them.";
	title2 "Except for Non-Radiological Progression.";

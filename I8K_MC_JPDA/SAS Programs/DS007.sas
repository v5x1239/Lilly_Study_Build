...
data DS007;
	...
	set fin;
	if datepart(AEENDAT) <= datepart(DSSTDAT);
...
/*Print DS007*/ * Replace AA001 with report name;
...
title2 "If  status is Adverse Event, then AE Stop date  for all non-serious events must be less or equal Discontinuation Date or Ongoing.";
  proc print data=DS007 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

...
proc sql;
	...
	b.lbtm_
	from  (select *, b.VISDAT from clntrial.EC1001_D a left join (select * from clntrial.SV1001_D where BLOCKID='802') b
	on a.SUBJID=b.SUBJID where a.page='EC1001_F1' and a.BLOCKID='801') a inner join lb b
	on a.SUBJID=b.SUBJID 
	where RQSTNTYP='ECG' and (CLLCTDT between datepart (ECSTDAT) and datepart (visdat)) and datepart(a.MERGE_DATETIME) > input("&date",Date9.)
	order by SUBJID;
quit;

...
/*Add code here*/
proc sort data=clntrial.EC1001_D out=ec (keep=SUBJID ECENDAT);
	by SUBJID ECENDAT;
	where page='EC1001_F6';
run;
data ec1;
	set ec;
	by SUBJID ECENDAT;
...
proc sql;
	...
	b.ECENDAT, 
	case 
	when not missing (RSDAT) and not missing (ECENDAT) then datepart (RSDAT) - datepart (ECENDAT)

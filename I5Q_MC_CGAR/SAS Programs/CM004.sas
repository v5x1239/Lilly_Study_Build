...
proc sql;
create table CM004 as 
select a.MERGE_DATETIME,b.SITEMNEMONIC,a.subjid,a.CMSPID,a.CMTRT,a.CMTRADNM,a.CMDECOD
from clntrial.CM1001 a left join clntrial.DM1001c b
	on a.SUBJID=b.SUBJID
	where datepart(a.MERGE_DATETIME) > input("&date", ? Date9.) and a.CMDECOD eq ''
...
/*Print CM004*/ * Replace AA001 with report name;
...
title2 "If 'Adverse Event' is selected as Indication, the medication must be appropriate to treat the event assigned.";
  proc print data=CM004 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

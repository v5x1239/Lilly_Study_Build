...
run;
proc sql;
create table DA041 as 
select /*merge_datetime,*/ SUBJID, BLOCKID, DASPID, DADAT_DISPAMT,DADAT_RETAMT 
from fin
order by SUBJID
;
quit;

/*Print DA041*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "DA_DISPENSE (Drug Accountability: Dispensed)";
title2 "Date dispensed should not be equal to date returned.";
  proc print data=DA041 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

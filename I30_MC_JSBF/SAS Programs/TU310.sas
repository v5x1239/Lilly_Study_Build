...
proc sql;
	...
	a.BLOCKID, b.TRLNKID,b.TRSPID
	from clntrial.TU3001a a left join clntrial.TU3001b b
	on a.SUBJID=b.SUBJID
	order by a.SUBJID,b.TRLNKID,b.TRSPID; 
quit;
data lag;
	set tu;
	sub = lag(SUBJID);
	trl = lag(TRLNKID);
	trp = lag(TRSPID);
	if sub eq SUBJID and TRLNKID gt trl and trp gt TRSPID;
run;
proc sql;
	create table TU310 as 
	select /*merge_datetime,*/ SUBJID, BLOCKID, TRLNKID,TRSPID
	from lag 
/*	where datepart(MERGE_DATETIME) > input("&date", ? Date9.) */
	order by SUBJID,TRLNKID,TRSPID;
quit;

	
/*Prem, keep the below code handy, this is the ouptut piece in JReview.*/

/*Print TU310*/ * Replace AA001 with report name;
...
title2 "Assessment number must be unique and sequential across assessments";
  proc print data=TU310 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

...
proc sql;
	create table TU as 
	select /*a.merge_datetime,*/ a.SUBJID,/*b.TULNKID_NONTARGET,*/ b.TRSPID, b.TRDAT
...
proc sql;
	...
	select /*a.merge_datetime,*/ a.SUBJID,a.DSDECOD,/*b.TULNKID_NONTARGET,*/ b.TRSPID, b.TRDAT,b.date
...
proc sql;
	create table TU318 as 
	select /*merge_datetime,*/SUBJID,DSDECOD,/*TULNKID_NONTARGET,*/TRSPID,TRDAT
	from fin 
/*	where datepart(MERGE_DATETIME) > input("&date", ? Date9.) */
	order by SUBJID,DSDECOD,/*TULNKID,*/TRSPID;
quit;

	
/*Prem, keep the below code handy, this is the ouptut piece in JReview.*/

/*Print TU318*/ * Replace AA001 with report name;
...
title2 "Look for missing tumor data for patients who have not died/Lost to Follow-up and don't have objective Progressive Disease reported and haven't had a scan within 42 (+/- 7) days since their last scan";
  proc print data=TU318 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

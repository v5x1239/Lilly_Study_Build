rem		
rem 	Usage : sqlplus <trialuser>/<password>@<database> @updateSectionItem_OVRLRESP_CMPD_RCHI_F1_2.sql
Set echo off;
set pagesize 0;
set linesize 500;
set head off;
...
spool updateSectionItem_OVRLRESP_CMPD_RCHI_F1_2.xml
...
and s.sectionrevisionnumber = (select distinct s2.sectionrevisionnumber from 	pf_section s2,
				...
				where s.sectionid=s2.sectionid
				  ...
				  and h.editiondescription = 'H9H_MC_JBAV 2.0.20')
/

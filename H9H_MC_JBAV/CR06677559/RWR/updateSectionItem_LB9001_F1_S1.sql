rem		
rem 	Usage : sqlplus <trialuser>/<password>@<database> @updateSectionItem_LB9001_F1_S1.sql
Set echo off;
set pagesize 0;
set linesize 500;
set head off;
...
spool updateSectionItem_LB9001_F1_S1.xml
...
from pf_item i,
     pf_section s
where i.itemrefname='LBNAM_LB9001_F1'
and i.itemrevisionnumber = (select max(i2.itemrevisionnumber) from pf_item i2 where i.itemid=i2.itemid)
and s.sectionrefname='igLB9001_F1_S1'
and s.sectionrevisionnumber = (select distinct s2.sectionrevisionnumber from 	pf_section s2,
										pf_pagesection d, 

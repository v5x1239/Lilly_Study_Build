...
proc sql;
	...
	select distinct STUDY, SITEMNEMONIC, r.SUBJID,TRSPID,RSSPID,TULNKID,TRPERF,TRDAT,RSDAT,TULOC1 as TULOC,
...
left join (select subjid, rsspid from 
	(select s.*, 
		case 
			...
			('NON CR OR NON PD', 'NOT ALL EVALUATED', 'NON-CR OR NON-PD', 'CR') and  v.TULNKID is null then 'x' 
...
left join (select distinct subjid, rsspid from 
	...
	case 
		...
		upper (NTRGRESP) in  ('NON CR OR NON PD', 'NOT ALL EVALUATED', 'NOT ASSESSED', 'NON-CR OR NON-PD', 'CR') or v.TULNKID  is null) or 

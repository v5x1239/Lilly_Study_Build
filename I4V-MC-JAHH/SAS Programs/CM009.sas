...
proc sort data=clntrial.CM1001_ALL out= CM1001;
...
proc sql;
	create tableCM_LAP as select a.*, b.CMSTDAT as CMST, b.CMENDAT as CMED,
	(case
...
proc sql;
	create table CM009 as select 
/*	strip(put(datepart(MERGE_DATETIME),date9.)) as MERGE_DATETIME,*/
	SUBJID,
	CMSPID,
	CMTRT,
	CMTRADNM,
	datepart(CMSTDAT) format =date9.,
	datepart(CMENDAT) format =date9.

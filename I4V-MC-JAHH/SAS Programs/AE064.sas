...
CODE NAME (required)              : AA064.sas
PROJECT NAME (required)           : I4V_MC_JAHH
DESCRIPTION (required)            : AA064 - Place holder for Report name - Add descripton here
...
;
	...
	Where upcase(Strip(AEOUT)) eq 'RECOVERED/RESOLVED'
;
	Create Table AE064 as Select 
		A.SUBJID, 
		A.BLOCKID, 
		A.AETERM,
		A.AEGRPID,
...
ods csv file=&irfilcsv trantab=ascii;
  title1 "If Event Outcome is 'Recovered',there must not be any entry with the same AEGRPID and Start date after that entry's End Date";

  proc print data=AE064 noobs WIDTH=min;
    var _all_;
  run;

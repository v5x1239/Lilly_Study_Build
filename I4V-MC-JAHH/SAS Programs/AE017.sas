...
DESCRIPTION (required)            : AE017 - Events within the same AEGRPID must have the same AEDECOD
...
Proc Sql;
	Create Table AE as Select
		SUBJID, AETERM, AEGRPID, AEDECOD

	From Clntrial.AE3001A where AEGRPID ne .

	Group by AETERM, AEDECOD

	Having count(AEDECOD) GE 2

	order by AEGRPID, AEDECOD
;
...
ods csv file=&irfilcsv trantab=ascii;
  title1 "Events within the same AEGRPID must have the same AEDECOD";

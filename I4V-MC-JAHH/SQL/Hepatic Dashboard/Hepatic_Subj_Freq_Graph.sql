SELECT Count(Category) as OccurancePerSubjid, SUBJID
            FROM
            (
    ...
    FROM (SELECT a.SUBJID, a.TEST, a.RESULT, a.HIGH, a.DATETIME 
            ...
            (SELECT TO_CHAR (a.SUBJID) AS SUBJID, A.BLOCKID, A.PAGE, A.PAGERPT,
                          ...
                          AS DATETIME
                             ...
                             OR ((TEST in ('E13','G01') and RESULT >= HIGH*2) and ((RESULT_lag >= HIGH_lag*2) or (RESULT_TYPE_lag >= HIGH_TYPE_lag*2)))) ) LABS
            )
            GROUP BY SUBJID
   UNION ALL
   SELECT count(Category) as OccurancePerSubjid, SUBJID
            FROM
       (SELECT ' ' Category, NULL SUBJID FROM DUAL)
...
CODE NAME (required)              : LS512.sas
PROJECT NAME (required)           : H9H-MC-JBAS
DESCRIPTION (required)            : Ensure there is a response for every assessment and sequences are logical per response criteria.  Also ensure there are not responses without lesion data to support them, unless non-radiological progression is reported.
...
/*Print LS512*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "Ensure there is a response for every assessment and sequences are";
title2 "logical per response criteria. Also ensure there are not responses";
title3 "without lesion data to support them, unless non-radiological progression is reported.";

  proc print data=LS512 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

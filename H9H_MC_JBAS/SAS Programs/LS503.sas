...
CODE NAME (required)              : LS503.sas
PROJECT NAME (required)           : H9H-MC-JBAS
DESCRIPTION (required)            : Ensure all location text fields are blank (i.e., text field has been resolved to a valid location from codelist which has been entered in the location code).
...
/*Print LS503*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "Ensure all location text fields are blank (i.e., text field";
title2 "has been resolved to a valid location from codelist which has";
title3 "been entered in the location code).";
  proc print data=LS503 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

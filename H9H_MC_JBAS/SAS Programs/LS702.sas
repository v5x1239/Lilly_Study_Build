...
CODE NAME (required)              : LS702.sas
PROJECT NAME (required)           : H9H-MC-JBAS
DESCRIPTION (required)            : Date of Response Assessment must be equal to a Date of Tumor Measurement with same assessment number in Tumor evaluation CRFs. 
...
proc sort data=recist (RENAME = (RSSPID = TRSPID));
	by subjid RSSPID;
run;

data LS702;
	merge recist (in=a) LS702a (in=b);
	by subjid RSSPID;
	if a;
	if RSDAT  ne . and TRSTDAT ne . and RSDAT ne TRSTDAT;
run;

/*Print LS702*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "Date of Response Assessment must be equal to a Date"; 
title2 "of Tumor Measurement with same assessment number";
title3 " in Tumor evaluation CRFs. ";

  proc print data=LS702 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

...
CODE NAME (required)              : LS517.sas
PROJECT NAME (required)           : H9H-MC-JBAS
DESCRIPTION (required)            : Ensure Response is appropriate for the lesions assessed. 
...
data LS517;
	merge LS517a (in=a) recist (in=b);
	by subjid;
	if a;
run;

/*Print LS517*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "Ensure Response is appropriate for the lesions assessed.";

  proc print data=LS517 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

...
CODE NAME (required)              : LS514.sas
PROJECT NAME (required)           : H9H-MC-JBAS
DESCRIPTION (required)            : Look for non SF patients with no lesion data recorded, but have a V0 present
...
data LS514;
	merge LS514a (in=a) recist (in=b);
	by subjid;
	if a;
	if TULNKID eq '';
run;

/*Print LS514*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "Look for non SF patients with no lesion data recorded, but have a V0 present";

  proc print data=LS514 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

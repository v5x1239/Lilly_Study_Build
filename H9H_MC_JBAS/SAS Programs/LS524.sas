...
CODE NAME (required)              : LS524.sas
PROJECT NAME (required)           : H9H-MC-JBAS
DESCRIPTION (required)            : Check for consistency across assessments (lesions not 10x size in previous assessment); include check of RECIST lesion measurements should be larger than mRECIST lesion if in same location. 
...
/*Print LS524*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "Check for consistency across assessments (lesions not 10x size in";
title2 " previous assessment) include check of RECIST lesion measurements";
title3 " should be larger than mRECIST lesion if in same location.";

  proc print data=LS524 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

...
CODE NAME (required)              : LS523.sas
PROJECT NAME (required)           : H9H-MC-JBAS
DESCRIPTION (required)            : Date of a new lesion assessment is after any dates of lesion already entered at previous assessments.
...
proc sql;
	...
	where a.subjid=b.subjid and a.page=b.page and a.PAGERPT=b.PAGERPT;
quit;
data new (RENAME = (TUDAT = NTUDAT));
	set target1a target3;
run;

data les;
	set  target1 target2 Ntarget1 ntarget2 clntrial.RS1001 (DROP=RSDATDD RSDATMO RSDATYY) clntrial.IRRC1001 (DROP=RSDATDD RSDATMO RSDATYY);
run;
...
/*Print LS523*/ * Replace AA001 with report name;
ods csv file=&irfilcsv trantab=ascii;
  title1 "Date of a new lesion assessment is after any dates of lesion";
title2 " already entered at previous assessments.";


  proc print data=LS523 noobs WIDTH=min; 
    var _all_;
  run;
ods csv close;

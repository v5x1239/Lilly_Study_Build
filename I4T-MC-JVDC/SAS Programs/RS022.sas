...
set clntrial.RS1001;
spid = RSSPID;
if page eq 'RS1001_F1' and TRGRESP in ('NOT ASSESSED','NOT EVALUABLE','NE'); 
...
data fin;
set rstu;
if TRGRESP ne '' and TUMSTATT in ('NOT ASSESSABLE','RESULT NOT MEASURED') 
then flag = '#';
run;
...
run;
proc sql;
create table mer as select subjid,count(subjid) as cnt,flag from fin1
...
set all1;
/*if cnt eq .;*/
keep SITEMNEMONIC subjid RSSPID TRSPID TRLNKID TRGRESP TUMSTATT;
run;
Proc sort;
by subjid;
run;

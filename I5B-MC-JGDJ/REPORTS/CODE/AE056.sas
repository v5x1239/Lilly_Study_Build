...
if EXTRTINT = 'Y' and AEGRPRL2 ne '';
keep SUBJID PAGE1 BLOCKID EXTRTINT AEGRPRL2 EXSTDAT1 EXENDAT1 id;
run;
proc sort;
by subjid ID;
run;
...
run;
data AE056;
retain SITEMNEMONIC SUBJID PAGE BLOCKID EXTRTINT AEGRPRL2 EXSTDAT EXENDAT AEGRPID AESPID AETERM AEDECOD AESTDAT AEENDAT AEONGO AEREL;
set final;
page = page1;
AESTDAT = AESTDAT1;
AEENDAT = AEENDAT1;
...
SITEMNEMONIC = 'Site Number' SUBJID = 'Subject Number' PAGE = 'Form Name' BLOCKID = 'Visit' EXTRTINT = 'Treatment Interrupted'
AEGRPRL2 = 'Adverse Event' EXSTDAT = 'Treatment Start Date' EXENDAT = 'Treatment End Date';
keep SITEMNEMONIC SUBJID PAGE BLOCKID EXTRTINT AEGRPRL2 EXSTDAT EXENDAT AEGRPID AESPID AETERM AEDECOD AESTDAT AEENDAT AEONGO AEREL;
run;
...
proc print data=AE056 noobs label WIDTH=min;
    var _all_;
  run;

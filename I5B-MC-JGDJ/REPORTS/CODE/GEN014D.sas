...
CODE NAME (required)              : GEN014D.sas
...
run;
proc sql;
create table ds as select SUBJID,'DS_SUM_FU' AS PAGENAME,datepart(DSSTDAT) as DSSTDAT format date9. from
clntrial.DS1001 where DSSTDAT ne . and page = 'DS1001_F4'  
order by SUBJID;
run;
%Macro dt(dsn,dt);
data dt;
...
SET MER;
LENGTH FLAG $50.;
IF DSDECOD IN ('DEATH', 'WITHDRAWAL BY SUBJECT','STUDY TERMINATED BY IRB / ERB', 'STUDY TERMINATED BY SPONSOR',
'LOST TO FOLLOW UP') and DATE NE . AND DSSTDAT NE . AND DATE GT DSSTDAT THEN FLAG = 'DSSTDAT is less than the date';
RUN;
PROC SORT;
BY SUBJID;
RUN;
...
proc print data=GEN014D noobs label WIDTH=min;
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set GEN014D nobs=NUM;
if NUM=0 then do;

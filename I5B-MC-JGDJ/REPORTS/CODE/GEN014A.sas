...
CODE NAME (required)              : GEN014A.sas
...
run;
proc sql;
create table ds as select SUBJID,'DS_SUM_SF' AS PAGENAME,datepart(DSSTDAT) as DSSTDAT format date9. from
clntrial.DS1001 where DSSTDAT ne . and page = 'DS1001_F1'
order by SUBJID;
run;
%Macro dt(dsn,dt);
data dt;
...
proc print data=GEN014A noobs label WIDTH=min;
    var _all_;
  run;
...
data _null_;
file print;
if 0 then set GEN014A nobs=NUM;
if NUM=0 then do;

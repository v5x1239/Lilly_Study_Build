	...
	create table CM007 as
		select merge_datetime,SUBJID, CMSPID,  CMTRT, CMSTDAT, CMONGO, CMENDAT, CMINDC, CMAEGRPID4,CMMHNO4, CMDOSE, CMDOSEU, CMDOSFRQ, CMROUTE, CMDECOD, CMTRADNM, CMCLAS, CMCLASCD
			from clntrial.CM1001_d
				where datepart(MERGE_DATETIME) > input("&date",Date9.) and CMDECOD in ('ACE INHIBITORS', 'ACETAZOLAMIDE', 'ACUPUNCTURE', 'ANGIOTENSIN RECEPTOR INHIBITORS', 'BENZODIAZEPINES'
					'BETA-BLOCKERS', 'CALCIUM CHANNEL BLOCKERS', 'CHIROPRACTIC PROCEDURE', 'CLONIDINE', 'CYCLANDELATE', 'CYPROHEPTADINE', 'GUANFACINE', 'NITRATES', 'NRI'
					'SNRI', 'SSRI', 'TENS DEVICE', 'TIZANIDINE', 'TRAZODONE');
quit;

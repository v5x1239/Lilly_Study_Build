...
proc sql;
	...
	select distinct a.merge_datetime, a.SUBJID, a.EXSTDAT, input(strip(strip(a.EXSTTIMHR)||':'||strip(a.EXSTTIMMI)),time8.)
	as EXSTTIM format time8., b.CLLCTDT, b.CLLCTTM
	from clntrial.EX1001_D a inner join clntrial.LABRSLTA b
	on a.SUBJID=b.SUBJID and datepart(a.EXSTDAT)=datepart(b.CLLCTDT)
	where calculated EXSTTIM lt input(strip(substr(put(b.CLLCTTM,best.),1,2)||':'||substr(put(b.CLLCTTM,best.),3,2)),time8.)
